				...
				if !path_instance.has(null):
					if !path_instance.has(file_name):
						...
						match type:
							"","Text":
								if content != "":
									path_instance[file_name] = content
							"Number":
								if content != "":
									if content.is_valid_int():
										path_instance[file_name] = int(content)
									elif content.is_valid_float():
										path_instance[file_name] = float(content)
									else:
										...
										path_instance[file_name] = 0
								else:
									path_instance[file_name] = 0
							"Boolean":
								if content != "" or content != "false":
									if content == "true":
										path_instance[file_name] = true
									else:
										append_text("[color=RED]" + tr("error.parameter_type_mismatch") + "[/color] " + tr("error.parameter_type_mismatch.expect_got") \
											% [tr("error.type.boolean"),content,"[i]false[/i]"])
										path_instance[file_name] = false
								else:
									path_instance[file_name] = false
							_:
